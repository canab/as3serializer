package garbuz.serialization
{
	internal final class Types
	{
		public static const T_NULL:int = 0;

		public static const T_TRUE:int = 1;
		public static const T_FALSE:int = 2;

		public static const T_STRING:int = 3;
		public static const T_DOUBLE:int = 4;
		public static const T_DATE:int = 5;
		public static const T_MAP:int = 6;
		public static const T_ARRAY:int = 7;
		public static const T_OBJECT:int = 8;

		public static const T_UINT1:int = 11;
		public static const T_UINT2:int = 12;
		public static const T_UINT3:int = 13;
		public static const T_UINT4:int = 14;

		public static const T_NINT1:int = 21;
		public static const T_NINT2:int = 22;
		public static const T_NINT3:int = 23;
		public static const T_NINT4:int = 24;
	}
}
